...
void CString::operator+=(int number)
{
	int no = number, digits = 0, divisor = 1;;
	while (no>0)
	{
		no = no/10;
		digits = digits + 1;
...
void CString::operator+=(double number)
{
	*this += (int)number;
	this->concatEqual('.');
